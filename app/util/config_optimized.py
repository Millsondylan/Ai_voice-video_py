...
# OPTIMIZED Default configuration values for FAST RESPONSE and NOISE REJECTION
DEFAULT_CONFIG: Dict[str, Any] = {
    "wake_word": "hey glasses",
    "silence_ms": 800,  # FIX: Reduced from 1500 to 800 for FASTER response after speech ends
    "max_segment_s": 45,
    "frame_sample_fps": 2,
    "frame_max_images": 6,
    "video_width_px": 960,
    ...
    "chunk_samples": 320,
    "vad_aggressiveness": 3,  # FIX: Increased from 2 to 3 for BETTER noise rejection (0=least sensitive, 3=most selective)
    "pre_roll_ms": 500,  # FIX: Increased from 400 to 500 for better phrase beginning capture
    "wake_variants": ["hey glasses", "hey-glasses", "hay glasses", "a glasses"],
    "wake_sensitivity": 0.7,  # FIX: Increased from 0.65 to 0.7 for better wake word detection
    "tts_voice": None,
    "tts_rate": 175,
    # Porcupine wake word detection (optional, falls back to Vosk)
    "prefer_porcupine": True,
    "porcupine_sensitivity": 0.7,  # FIX: Increased from 0.65 to 0.7
    "porcupine_keyword_path": None,
    # FIX: Advanced speech capture settings for complete word capture AND fast response
    "min_speech_frames": 3,  # FIX: Reduced from 5 to 3 - allows faster detection of silence
    "tail_padding_ms": 200,  # FIX: Reduced from 400 to 200 - less dead air after speech ends
}
